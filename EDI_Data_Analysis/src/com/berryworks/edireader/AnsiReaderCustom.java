 ...
 * with parsing.
 */
public class AnsiReaderCustom extends StandardReader {
	char[] cv;
	...
	protected String groupControl;
	protected String groupStandardCode;
	protected String groupStandardVersion;
...
/*		char[] cv;*/
		...
		String Date = getTokenizer().nextSimpleValue();
		...
		if (separator == -1) {
			// No repetition char is in effect. It is therefore safe to
			// interpret this next
			// element as the standardsId used through version 4010 of ANSI X12.
			standardsId = getFixedLengthISAField(1);
			getInterchangeAttributes().addCDATA(getXMLTags().getStandardsId(),
					standardsId);
		} else {
			// A repetition char is in effect, presumably previewed from this
			// ISA segment we
			// are now parsing. Therefore, we treat this field in accordance
			// with version 4020
			// or later where it repetition character instead of a standardsId.
			// Temporarily disable the repetition char so that we can parse over
			// this element
			// as normal data.
			/**
			 * Srini Corrected code to pick both standard id 
			 */
		...
		// Integer.toString(separator);
		
		String versionId = getFixedLengthISAField(5);
		setInterchangeControlNumber(getFixedLengthISAField(9));
		
		
		String ackRequ = getFixedLengthISAField(1);
		String useIndicator =  getFixedLengthISAField(1);
		Token nextToken = getTokenizer().nextToken();
		String elementSep = " ";
		if (!nextToken.getValue().isEmpty())
			elementSep = nextToken.getValue();
		...
		while (getTokenizer().nextToken().getType() != Token.TokenType.SEGMENT_END)
			if (getTokenizer().getElementInSegmentCount() > 30)
				throw new EDISyntaxException(TOO_MANY_ISA_FIELDS,
						getTokenizer());

		// Now make the the callbacks to the ContentHandler
		// startElement(getXMLTags().getInterchangeTag(),
		// getInterchangeAttributes());

		startElement(getXMLTags().getInterchangeTag(),
				getInterchangeAttributes());
		getInterchangeAttributes().clear();
		createElement("ISA01",authQual);
		createElement("ISA02",authInfo);
		createElement("ISA03",securityQual);
		createElement("ISA04",securityInfo);
		createElement("ISA05",fromQual);
		createElement("ISA06",fromId);
		createElement("ISA07",toQual);
		createElement("ISA08",toId);
		createElement("ISA09", Date);
		createElement("ISA10",controlTime);
		createElement("ISA11",standardsId);
		createElement("ISA12",versionId);
		createElement("ISA13",getInterchangeControlNumber());
		createElement("ISA14",ackRequ);
		createElement("ISA15", useIndicator);
		createElement("ISA16",elementSep);
	endElement(getXMLTags().getInterchangeTag());
		...
		if (getGroupCount() != (n = getTokenizer().nextIntValue())) {
			GroupCountException countException = new GroupCountException(
					COUNT_IEA, getGroupCount(), n, getTokenizer());
			setSyntaxException(countException);
			if (!recover(countException))
				throw countException;
		}
		String s;
		if (!(s = getTokenizer().nextSimpleValue())
				.equals(getInterchangeControlNumber())) {
			InterchangeControlNumberException interchangeControlNumberException = new InterchangeControlNumberException(
					CONTROL_NUMBER_IEA, getInterchangeControlNumber(), s,
					getTokenizer());
	...
	protected void parseTA1(Token token) throws SAXException, IOException {
		...
		attributes.addCDATA(getXMLTags().getControl(),
				acknowledgedControlNumber);
	...
	protected String getFixedLengthISAField(int expectedLength, boolean enforce)
		...
		if (field.length() != expectedLength) {
			if (enforce)
				throw new EDISyntaxException(ISA_FIELD_WIDTH, expectedLength,
						field.length(), getTokenizer());
			else
	 ...
	 *
	 * @param token
	 *            parsed token that caused this method to be called
	...
	protected Token parseFunctionalGroup(Token token) throws SAXException,
		...
		try {
			...
			if (e.getMessage().startsWith("Mandatory")) {
				MissingMandatoryElementException missingMandatoryElementException = new MissingMandatoryElementException(
						MANDATORY_ELEMENT_MISSING,
						"at least one non-space character", "(empty)",
						getTokenizer());
		...
		groupStandardCode = getTokenizer().nextSimpleValue();
		groupStandardVersion = getTokenizer().nextSimpleValue();
		
		/*getGroupAttributes().addCDATA(getXMLTags().getStandardCode(),
				getTokenizer().nextSimpleValue());*/

		// Handle the groupVersion at the end of the segment. This is a bit
		// tricky since
		// the groupVersion may be omitted causing us to encounter the end of
		// segment earlier
		// than expected.
		Token t = getTokenizer().nextToken();
		if (t.getType() == Token.TokenType.SEGMENT_END) {
			...
			getGroupAttributes().addCDATA(getXMLTags().getStandardVersion(),
					groupVersion);
			getTokenizer().skipSegment();
		}
		...
		getGroupAttributes().clear();
		createElement("GS01",groupFunctionCode);
		createElement("GS02",groupSender);
		createElement("GS03",groupReceiver);
		createElement("GS04",groupDate);
		createElement("GS05",value);
		createElement("GS06",getGroupControlNumber());
		createElement("GS07",groupStandardCode);
		createElement("GS08",groupStandardVersion);
		...
		if (docCount != (n = getTokenizer().nextIntValue())) {
			TransactionCountException countException = new TransactionCountException(
					COUNT_GE, docCount, n, getTokenizer());
			setSyntaxException(countException);
			if (!recover(countException))
				throw countException;
		}
	...
	protected void createElement(String elementName, String value) throws SAXException
	{
		startElement(elementName, getGroupAttributes());
		cv = value.toCharArray();
	...
	protected Token parseDocument(Token token) throws SAXException, IOException {
		...
		String version = groupStandardVersion;
		/*if (version.length() > 6)
			version = version.substring(0, 6);*/
		/*String code = getGroupAttributes().getValue(
				getXMLTags().getStandardCode());*/
		String code=groupStandardCode;
		PluginController pluginController = PluginController.create("ANSI",
				documentType, code, version, getTokenizer());
		if (pluginController.isEnabled())
			/*getDocumentAttributes().addCDATA(getXMLTags().getName(),
					pluginController.getDocumentName());*/
		documentName = pluginController.getDocumentName();
		...
		getDocumentAttributes().clear();
		createElement("ST01",documentType);
		createElement("ST02",control);
		while ((t = getTokenizer().nextToken()).getType() != Token.TokenType.SEGMENT_END) {
			createElement( t.getElementId() , t.getValue());
		}
		...
		for (; toClose > 0; toClose--) {
			/**
			 * Srini - Modification on Loop creation Commenting Starting of Loop
			 * and creating a loop in different way </loop> to </l-XXX>
			 */
		...
		if (segCount != (n = getTokenizer().nextIntValue())) {
			SegmentCountException countException = new SegmentCountException(
					COUNT_SE, segCount, n, getTokenizer());
			setSyntaxException(countException);
			if (!recover(countException))
				throw countException;
		}
		...
		if (!(s = getTokenizer().nextSimpleValue()).equals(control)) {
			TransactionControlNumberException transactionControlNumberException = new TransactionControlNumberException(
					CONTROL_NUMBER_SE, control, s, getTokenizer());
		...
		getAckGenerator().generateTransactionAcknowledgment(documentType,
				control);
	...
	protected void parseBINSequence() throws SAXException, IOException {
		...
		try {
			...
			throw new EDISyntaxException(ErrorMessages.MISSING_UNO_LENGTH,
					getTokenizer());
		} catch (NumberFormatException e) {
			throw new EDISyntaxException(
					"BIN object length must be numeric instead of "
							+ lengthField, getTokenizer());
		}
		...
		new ContentHandlerBase64Encoder().encode(dataObject,
				getContentHandler());
		endElement(getXMLTags().getPackageTag());
	}

	private String parseStringFromNextElement() throws IOException,
			EDISyntaxException {
		...
		throw new EDISyntaxException(ErrorMessages.MANDATORY_ELEMENT_MISSING,
				getTokenizer());
	}
	...
	public void preview() throws EDISyntaxException, IOException {
		...
		setTerminatorSuffix(findTerminatorSuffix(buf,
				indexOf16thFieldSeparator + 3, 128));

		// Determine the repetition character. This changed in 4.6.5 to support
		// repetition chars
