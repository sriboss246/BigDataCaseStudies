 ...
 * <p/>
 * An EDI parser delegates the job of detecting segment loop boundaries to a
 * PluginController. This allows the EDI parsers for ANSI and EDIFACT to be
 * fully consistent with their use of plugins and focus on the specifics of the
 * particular EDI standard.
 * <p/>
 * This PluginControllerImpl provides the normal segment loop support based on
 * LoopDescriptors in Plugins. It is possible, however, to extend this behavior
 * by subclassing PluginControllerImpl and Plugin. A ValidatingPlugin is one
 * example, which provides for certain EDI validation rules, beyond those
 * applied by normal EDIReader parsing, to be applied while a document is being
 * parsed. Another example is a FilteringPlugin, which allows a plugin to
 * provide custom logic to filter out certain LoopDescriptors based on run-time
 * decisions.
 * 
...
public class PluginControllerImpl extends PluginController {
	...
	public static String plugginKey;
	...
	/**
	 * This constructor is not for general use because create methods are
	 * provided as factory methods to instantiate a new instance of a
	 * PluginController.
	 * 
	 * @param standard
	 * @param tokenizer
	...
	/**
	 * Creates a new instance of a PluginController, selecting a plugin based on
	 * the standard and type of document.
	 * 
	 * @param standard
	 * @param docType
	 * @param tokenizer
	 * @return instance
	 */
	public static PluginControllerImpl create(String standard, String docType,
			Tokenizer tokenizer) {
	...
	/**
	 * Creates a new instance of a PluginController, selecting a plugin based on
	 * the standard, the type of document, and the version and release
	 * characteristics.
	 * <p/>
	 * This factory method supports version-specific plugins for a given type of
	 * document. If first tries to load a plugin specific to a particular
	 * release and version using the naming convention for plugin class names.
	 * If no version-specific plugin is available, it uses the other factory
	 * method to create a plugin based simply on the standard and type.
	 * 
	 * @param standard
	 * @param docType
	 * @param docVersion
	 ...
	 */
	public static PluginControllerImpl create(String standard, String docType,
			String docVersion, String docRelease, Tokenizer tokenizer) {
	...
	public boolean transition(String segmentName) throws EDISyntaxException {
		...
		LoopDescriptor newDescriptor = plugin.query(segmentName,
				loopStack.toString(), loopDescriptor.getNestingLevel());

		if (debug)
			trace("considering segment " + segmentName + " using descriptor "
					+ newDescriptor);
		...
		if (Plugin.CURRENT.equals(newLoopName)
				&& newDescriptor.getNestingLevel() == loopDescriptor
						.getNestingLevel()) {
			if (debug)
				trace("resuming current loop without transition");
		} else {
			if (debug)
				trace("transitioning to level "
			...
			if (debug)
				trace("closing " + numberOfLoopsClosed + " loops");

			if ((numberOfLoopsClosed < 0)
					|| (numberOfLoopsClosed > loopDescriptor.getNestingLevel()))
				throw new EDISyntaxException(
						"Improper sequencing noted with segment " + segmentName,
						tokenizer);
				...
				for (int i = 0; i < numberOfLoopsClosed; i++) {
					...
					if (debug)
						trace("popped " + completedLoop + " off the stack");
				}
			}
			...
			if (resumeLoop) {
				if (debug)
					trace("resuming loop at level "
							+ loopDescriptor.getNestingLevel() + " with name "
				...
				if (loopDescriptor.getNestingLevel() == 0
					...
					if (debug)
						trace("special legacy case: " + loopDescriptor);
					loopStack.setBottom(new LoopContext(loopDescriptor
							.getName().substring(1)));
				}
			} else {
				loopStack.push(createLoopContext(loopDescriptor.getName(),
						plugin, loopStack.toString()));
				if (debug)
					trace("pushed " + loopDescriptor.getName()
							+ " onto the stack");
			}
		}
	...
	/**
	 * Used only within the internal implementation of this class and its
	 * subclasses.
	 * 
	 * @param name
	 * @param plugin
	 ...
	 * <p/>
	 * The default implementation provide by this class returns true, indicating
	 * that the LoopDescriptor is valid, for any non-null LoopDescriptor. The
	 * primary purpose of this method is to provide a "hook" so that a subclass
	 * of PluginController can override this method and apply custom logic.
	 * 
	 * @param descriptor
	 * @param segmentName
	 * @param tokenizer
	...
	protected boolean validateDescriptor(LoopDescriptor descriptor,
			String segmentName, Tokenizer tokenizer) throws EDISyntaxException {
	 ...
	 * <p/>
	 * The default implementation is empty. A PluginController subclass can
	 * override this method in order to apply its own validation policies.
	 * 
	 * @param completedLoop
	 * @throws EDISyntaxException
	 */
	protected void validateCompletedLoop(LoopContext completedLoop)
			throws EDISyntaxException {
	}

	/**
	 * Validates the appearance of a segment, given the selected LoopDescriptor
	 * and the current LoopStack.
	 * <p/>
	 * The default implementation always returns true. The primary purpose of
	 * this method is to provide a "hook" for subclasses to perform a particular
	 * type of validation.
	 * 
	 * @param descriptor
	 * @param loopStack
	 ...
	 * @throws EDISyntaxException
	 */
	protected boolean validateSegment(LoopDescriptor descriptor,
			LoopStack loopStack, Tokenizer tokenizer) throws EDISyntaxException {
		return true;
	}
	...
	/**
	 * Returns true if the most recent loop transition was to resume an outer
	 * loop.
	 * 
	 * @return
	 */
	...
	/**
	 * Used within the implementation of PluginController to find a plugin for a
	 * given standard, document type, version, and release. If no matching
	 * plugin is found, it returns null. Plugins are cached so that once a
	 * plugin is loaded it can be quickly found again without using the class
	 * loader.
	 * 
	 * @param standard
	 * @param docType
	 ...
	 * @return
	 */
	protected static Plugin loadPlugin(String standard, String docType,
			String docVersion, String docRelease) {
		Plugin result = null;
		String key = standard + "_" + docType + "_" + docVersion + "_"
				+ docRelease;
		plugginKey = key;
		if (pluginCache.containsKey(key)) {
			...
			if (docVersion != null && docVersion.length() > 0
					&& docRelease != null && docRelease.length() > 0) {
				if (suffix != null && suffix.length() > 0)
					result = lookForSpecificPlugin(standard, docVersion + "_"
							+ docRelease + "." + standard + "_" + docType + "_"
							+ suffix);

				if (result == null)
					result = lookForSpecificPlugin(standard, docVersion + "_"
							+ docRelease + "." + standard + "_" + docType);

				if (suffix != null && suffix.length() > 0)
					result = lookForSpecificPlugin(standard, docType + "_"
							+ docVersion + "_" + docRelease + "_" + suffix);

				if (result == null)
					result = lookForSpecificPlugin(standard, docType + "_"
							+ docVersion + "_" + docRelease);
			}
	 ...
	 * @return
	 */
	protected static Plugin lookForSpecificPlugin(String standard,
			String docType) {
		...
		try {
			...
			if (debug)
				trace("plugin for " + docType
						+ " caused IllegalAccessException" + e);
		}
	 ...
	 */
	protected static Plugin getInstance(String standard, String docType)
	 ...
	 * <p/>
	 * The default value is "com.berryworks.edireader.plugin" but this can be
	 * changed by setting the system property EDIREADER_PLUGIN_PACKAGE. In this
	 * way, a user can develop their own plugins and using a package naming
